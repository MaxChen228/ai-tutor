...
def generate_question_batch(weak_points_str, num_review):
    """
    【v5.16 智慧提示版】: (複習題) 要求 AI 產出包含 new_sentence 和 hint_text 的 JSON 物件。
    """
    system_prompt = f"""
            ...
            **你的核心工作原則：**
            ...
            2.  **情境創造**：你要創造一個全新的、自然的、合乎邏輯的中文情境，使得「正確用法」是這個情境下最貼切的翻譯。
            3.  **絕對保密**：在你的題目中，絕對不能出現「正確用法」的任何英文字眼。

            **【重要指令】輸出格式**
            你必須嚴格回傳一個 JSON 物件。根部有一個 "questions" key，其 value 是一個列表。
            列表中的每一個問題物件都必須包含以下兩個 key：
            1. "new_sentence": (string) 你設計的中文考題。
            2. "hint_text": (string) 這道題的核心考點，也就是你本次主要想測驗的那個「正確用法」。
    ...
    user_prompt = f"""
        **【學生具體知識點弱點報告】**
        ...
        請務必記得，你的輸出必須是包含 "new_sentence" 和 "hint_text" 的 JSON 格式。
        """
    try:
...
def generate_new_question_batch(num_new, difficulty, length):
    """
    【v5.16 智慧提示版】: 要求 AI 產出包含 new_sentence 和 hint_text 的 JSON 物件。
    """
    try:
    ...
    **指令三：嚴格輸出**
    ...
    每一個問題物件都必須包含以下兩個 key：
    1. "new_sentence": (string) 你設計的中文考題。
    2. "hint_text": (string) 你本次出題時，主要使用的那個「指定句型」。
    ...
    {{
        "questions": [
            {{ 
                ...
                "hint_text": "再...也不為過: cannot + V + too much / adv."
            }}
        ]
    }}
    """
...
def get_tutor_feedback(chinese_sentence, user_translation, review_context=None, hint_text=None):
    ...
    if review_context:
        ...
        **【重要補充資訊】**
        這道題目的核心考點提示是：「{hint_text if hint_text else '無特定提示'}」。請你在分析時，特別留意學生對這個考點的掌握情況。
